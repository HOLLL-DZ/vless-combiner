...
# Создаём config.yaml, если его нет или он невалиден
CONFIG_FILE="$DEPLOY_DIR/config.yaml"
if [ ! -f "$CONFIG_FILE" ] || [ ! -s "$CONFIG_FILE" ] || ! grep -q "base_url" "$CONFIG_FILE"; then
...
YAML
    ...
    echo "⚠️ config.yaml уже существует и валиден — не перезаписываю"
fi
