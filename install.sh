...
# Автоматическое получение публичного IP (если возможно)
PUBLIC_IP=""
...
if [ -z "$PUBLIC_IP" ]; then
    echo "⚠️ Не удалось определить публичный IP. Использую тестовый адрес."
    PUBLIC_IP="test.com.net"
else
    echo "✅ Обнаружен IP: $PUBLIC_IP"
fi

# Спрашиваем адрес (домен или IP) — с предложенным значением
read -p "🌐 Введите ваш домен или IP-адрес сервера (по умолчанию: $PUBLIC_IP): " SERVER_ADDR
...
echo "   Чтобы обновить — замени app.py и шаблоны, затем:"
