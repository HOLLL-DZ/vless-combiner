...
DOMAIN="$SERVER_ADDR"
# Безопасная обработка пути админки: удаляем недопустимые символы
...
# Обновляем config.yaml под текущую установку
echo "🔧 Настраиваю config.yaml для вашего сервера..."
...
if [ ! -f "$DEPLOY_DIR/config.yaml" ] || [ ! -s "$DEPLOY_DIR/config.yaml" ]; then
    echo "⚠️ Не удалось скачать config.yaml с GitHub. Создаю вручную..."
